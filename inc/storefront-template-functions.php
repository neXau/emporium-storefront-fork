...
if ( ! function_exists( 'storefront_comment' ) ) {
	...
	function storefront_comment( $comment, $args, $depth ) {
		...
		<div class="comment-meta commentmetadata">
			...
			<?php printf( wp_kses_post( '<cite class="fn">%s</cite>', 'emporium' ), get_comment_author_link() ); ?>
			</div>
			<?php if ( '0' === $comment->comment_approved ) : ?>
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'emporium' ); ?></em>
				<br />
			<?php endif; ?>
		...
		?>
		<?php edit_comment_link( __( 'Edit', 'emporium' ), '  ', '' ); ?>
		</div>
		</div>
		<?php if ( 'div' !== $args['style'] ) : ?>
		</div>
	...
	function storefront_credit() {
		...
		if ( apply_filters( 'storefront_credit_link', true ) ) {
			if ( storefront_is_woocommerce_activated() ) {
				$links_output .= '<a href="https://woocommerce.com" target="_blank" title="' . esc_attr__( 'WooCommerce - The Best eCommerce Platform for WordPress', 'emporium' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with WooCommerce', 'emporium' ) . '</a>.';
			} else {
				$links_output .= '<a href="https://woocommerce.com/products/storefront/" target="_blank" title="' . esc_attr__( 'Storefront -  The perfect platform for your next WooCommerce project.', 'emporium' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with Storefront', 'emporium' ) . '</a>.';
			}
		}
...
if ( ! function_exists( 'storefront_primary_navigation' ) ) {
	...
	function storefront_primary_navigation() {
		?>
		<nav id="site-navigation" class="main-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Primary Navigation', 'emporium' ); ?>">
		<button id="site-navigation-menu-toggle" class="menu-toggle" aria-controls="site-navigation" aria-expanded="false"><span><?php echo esc_html( apply_filters( 'storefront_menu_toggle_text', __( 'Menu', 'emporium' ) ) ); ?></span></button>
			<?php
			wp_nav_menu(
				array(
...
if ( ! function_exists( 'storefront_secondary_navigation' ) ) {
	...
	function storefront_secondary_navigation() {
		if ( has_nav_menu( 'secondary' ) ) {
			?>
			<nav class="secondary-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Secondary Navigation', 'emporium' ); ?>">
				<?php
					wp_nav_menu(
						array(
...
if ( ! function_exists( 'storefront_skip_links' ) ) {
	...
	function storefront_skip_links() {
		?>
		<a class="skip-link screen-reader-text" href="#site-navigation"><?php esc_html_e( 'Skip to navigation', 'emporium' ); ?></a>
...
if ( ! function_exists( 'storefront_homepage_header' ) ) {
	...
	function storefront_homepage_header() {
		edit_post_link( __( 'Edit this section', 'emporium' ), '', '', '', 'button storefront-hero__button-edit' );
		?>
	...
	function storefront_page_content() {
		...
		<div class="entry-content">
			...
			<?php
				wp_link_pages(
					array(
						'before' => '<div class="page-links">' . __( 'Pages:', 'emporium' ),
						'after'  => '</div>',
					)
				);
			?>
	...
	function storefront_post_content() {
		...
		the_content(
			sprintf(
				...
				__( 'Continue reading %s', 'emporium' ),
...
if ( ! function_exists( 'storefront_post_meta' ) ) {
	...
	function storefront_post_meta() {
		...
		$posted_on = '
			...
			sprintf( __( 'Posted on %s', 'emporium' ), $output_time_string ) .
			'</span>';
		...
		if ( ! post_password_required() && ( comments_open() || 0 !== intval( get_comments_number() ) ) ) {
			$comments_number = get_comments_number_text( __( 'Leave a comment', 'emporium' ), __( '1 Comment', 'emporium' ), __( '% Comments', 'emporium' ) );
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'emporium' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'storefront_post_taxonomy' ) ) {
	...
	function storefront_post_taxonomy() {
		...
		$categories_list = get_the_category_list( __( ', ', 'emporium' ) );

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'emporium' ) );
		?>

		<aside class="entry-taxonomy">
			<?php if ( $categories_list ) : ?>
			<div class="cat-links">
				<?php echo esc_html( _n( 'Category:', 'Categories:', count( get_the_category() ), 'emporium' ) ); ?> <?php echo wp_kses_post( $categories_list ); ?>
			</div>
			<?php endif; ?>

			<?php if ( $tags_list ) : ?>
			<div class="tags-links">
				<?php echo esc_html( _n( 'Tag:', 'Tags:', count( get_the_tags() ), 'emporium' ) ); ?> <?php echo wp_kses_post( $tags_list ); ?>
			</div>
			<?php endif; ?>
		</aside>
...
if ( ! function_exists( 'storefront_paging_nav' ) ) {
	...
	function storefront_paging_nav() {
		...
		$args = array(
			'type'      => 'list',
			'next_text' => _x( 'Next', 'Next post', 'emporium' ),
			'prev_text' => _x( 'Previous', 'Previous post', 'emporium' ),
		);
...
if ( ! function_exists( 'storefront_post_nav' ) ) {
	...
	function storefront_post_nav() {
		$args = array(
			'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next post:', 'emporium' ) . ' </span>%title',
			'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous post:', 'emporium' ) . ' </span>%title',
		);
